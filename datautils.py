...
def get_c4(nsamples, seed, seqlen, model, tokenizer):
    #traindata = load_dataset(
        #'/sharedata/home/msy/code_llm_low_bit/llama_family/llm/dataset/allenai/c4', 'allenai--c4', data_files={'train': 'en/c4-train.00000-of-01024.json.gz'}, split='train'
    #)
    valdata= load_from_disk("/sharedata/home/msy/code_llm_low_bit/llama_family/llm/dataset/allenai/c4/validation/train")
    valenc =[]
    random.seed(seed)
    trainloader = []
    for _ in range(nsamples):
        while True:
            ...
            temp = tokenizer(valdata[i]['text'], return_tensors='pt')
            if temp.input_ids.shape[1] > seqlen:
                break
        i = random.randint(0, temp.input_ids.shape[1] - seqlen - 1)
        j = i + seqlen
        valenc.append(temp.input_ids[:, i:j])
